 ...
 * later.
 *
 * ## Every line opens with the word
 *
 * A disclaimer that reads as a caption gets skimmed as one. Opening with
 * "Disclaimer:" names what the sentence is before the reader has decided
 * whether to read it, which is the only moment the naming is worth anything —
 * and it makes the lines recognisable as the same class of thing across
 * surfaces that otherwise look nothing alike.
 *
 * It is applied by {@link disclaimer} rather than typed into each string
 * below, for the reason at the top of this file one level down: ten copies of
 * a word is nine chances to leave one behind, or to end up with "Disclaimer -"
 * on one surface and "Disclaimer:" on the rest.
 *
 * Putting it in the *string* rather than in `components/legal/disclaimer.tsx`
 * is deliberate too. Not every one of these renders through that component —
 * the app sidebar writes `FOOTER_COMPACT` into a plain `<p>` — and a prefix
 * that only appears when a particular component is used is a prefix that goes
 * missing exactly where nobody is looking. In the string, it travels with the
 * text wherever the text goes.
 */

/** Opens a disclaimer with the word, so the reader knows what they are reading. */
const disclaimer = (body: string) => `Disclaimer: ${body}`;
...
export const RECOMMENDATION = disclaimer(
 ...
 * price.
 */
export const ROI_OUTPUT = disclaimer(
...
export const ROI_INPUT = disclaimer(
...
export const FOOTER_COMPACT = disclaimer("Planning guidance, not financial or engineering advice.");
