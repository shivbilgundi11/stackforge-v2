...
it("turns Razorpay's POST callback into the confirmation-page GET", () => {
  const response = callback("https://app.stackforge.dev/checkout/callback?plan=team");
...
it("carries no plan through when Razorpay sent none", () => {
  expect(callback("https://app.stackforge.dev/checkout/callback").headers.get("location")).toBe(
    "/checkout/done",
  );
});
...
it("escapes a plan that would otherwise break out of the query", () => {
  const response = callback("https://app.stackforge.dev/checkout/callback?plan=a%26b%3Dc");
