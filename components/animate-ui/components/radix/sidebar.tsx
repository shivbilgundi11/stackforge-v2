...
// Stock Radix tooltip, not the Animate UI one. Animate UI's TooltipContent
// renders with `forceMount`, so in icon-collapsed mode every menu label mounts
// at once and spills out of the 48px rail. The rail only needs a plain label
// on hover, so there is no animation worth that trade.
...
function SidebarMenuButton({
  ...
  // Only mount the tooltip when the rail is actually collapsed. The upstream
  // registry gates it with `hidden` on TooltipContent, which works for stock
  // Radix (content is unmounted while closed) but not for Animate UI's
  // motion-driven content — there, every label renders permanently and spills
  // out of the 48px rail.
...
function SidebarMenuAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        // Increases the hit area of the button on mobile.
        "absolute top-1.5 right-1 z-1 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform peer-hover/menu-button:text-sidebar-accent-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        "after:absolute after:-inset-2 md:after:hidden",
