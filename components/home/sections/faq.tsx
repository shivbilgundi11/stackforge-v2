      ...
      <div className="grid gap-8 pt-7 lg:grid-cols-[minmax(0,14rem)_minmax(0,1fr)] lg:gap-16">
        <Reveal from="none" duration={0.6}>
          <p className="t-mono flex items-baseline gap-3 text-(--h-fg-45)">
            <span className="text-(--h-acc)">08</span>
            Questions
          </p>
        </Reveal>
        ...
        <div>
          ...
          <dl className="mt-14 border-t border-(--h-line)">
            {items.map((item, i) => {
              const expanded = open === i;
              const panelId = `faq-panel-${i}`;

              return (
                <div key={item.q} className="group border-b border-(--h-line)">
                  <dt>
                    <button
                      type="button"
                    ...
                    >
                      <span className="t-mono w-8 shrink-0 pt-2 text-(--h-fg-45)">
                      ...
                      <span
                        aria-hidden
                        className="relative mt-3 block size-4 shrink-0 text-(--h-acc)"
                      >
          ...
          <Reveal delay={0.1} className="mt-9 flex flex-wrap items-center gap-x-8 gap-y-3">
            <Link href="/faq" data-cursor="Read" className="u-link t-mono text-(--h-acc)">
              Read the full FAQ →
            </Link>
            <Link href="/contact" className="u-link t-mono text-(--h-fg-45)">
