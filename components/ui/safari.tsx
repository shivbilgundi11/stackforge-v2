...
/**
 * A Safari window frame for a product screenshot.
 *
 * Vendored from Magic UI (`shadcn add @magicui/safari`) rather than installed,
 * because three things about the upstream component are wrong for this
 * codebase and all three are in the parts a registry update would overwrite.
 * The toolbar — traffic lights, the address pill, the lock, and the eight
 * navigation glyphs — is upstream's artwork at upstream's coordinates, so a
 * future `diff` against the registry is still readable.
 *
 * ## 1. The screen is a slot, not an `imageSrc`
 *
 * Upstream renders a bare `<img>`. Every screenshot on this site goes through
 * `next/image` for the responsive `sizes` set, the lazy/`priority` split, and —
 * on the theme-aware surfaces — a light/dark pair swapped in CSS. Handing the
 * frame a `src` would have meant giving all of that up for a mockup border, so
 * the screen is `children` and the caller keeps its `<Image>`. `imageSrc` and
 * `videoSrc` still work for the plain case.
 *
 * ## 2. The height follows the screenshot
 *
 * Upstream locks the whole component to `aspect-ratio: 1203/753` and covers the
 * screen area, which is 1200x700 — 12:7. The captures here are 2880x1800 — 16:10.
 * Dropping a 16:10 image into a 12:7 hole crops about 7% off the bottom, and on
 * these captures the bottom 7% is the export bar: the row of buttons that makes
 * the point that the result is exportable. So the frame is a fixed-height
 * toolbar above a screen that is as tall as whatever is in it, and nothing is
 * cropped at any width.
 *
 * ## 3. The frame follows the ground it sits on
 *
 * Upstream hardcodes `fill-[#E5E5E5] dark:fill-[#404040]` — a frame that
 * follows the `.dark` class on `<html>`. The marketing site does not have one:
 * it alternates bone and ink chapters on a fixed palette, and `home.css` pins
 * `--line` and `--surface` per chapter (see the "Reused product figures" note
 * there). Reading those tokens instead means the frame is light on bone, dark
 * on ink, and still correct inside the product, with no `tone` prop to thread
 * through. The glyphs keep upstream's `#A3A3A3`, which is what upstream uses in
 * both themes anyway.
 */

/** Upstream's toolbar geometry, kept so its path data can be pasted verbatim. */
const SAFARI_WIDTH = 1202;
const TOOLBAR_HEIGHT = 52;

type SafariMode = "default" | "simple";

export interface SafariProps extends Omit<HTMLAttributes<HTMLDivElement>, "children"> {
  /** The address-bar text. Omitted leaves the pill empty, as upstream does. */
  url?: string;
  /** The screen. Preferred over `imageSrc` — see the note above. */
  children?: ReactNode;
  imageSrc?: string;
  videoSrc?: string;
  /** `simple` drops the eight navigation glyphs and keeps the address pill. */
  mode?: SafariMode;
}
...
export function Safari({
  ...
  const screen =
    children ??
    (videoSrc ? (
      <video className="block w-full" src={videoSrc} autoPlay loop muted playsInline preload="metadata" />
    ) : imageSrc ? (
      // eslint-disable-next-line @next/next/no-img-element -- the `children` slot is the `next/image` path; this is the plain fallback.
      <img className="block w-full" src={imageSrc} alt="" />
    ) : null);
  ...
  return (
    <div
      ...
      {...props}
    >
      {/* Decorative. The screenshot's own alt text carries the meaning, and an
          address bar read aloud before it would be noise. */}
      <svg
        viewBox={`0 0 ${SAFARI_WIDTH} ${TOOLBAR_HEIGHT}`}
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        aria-hidden
        className="block h-auto w-full"
      >
        <circle cx="27" cy="25" r="6" fill="#FB2C36" />
        <circle cx="47" cy="25" r="6" fill="#F0B100" />
        <circle cx="67" cy="25" r="6" fill="#00C950" />
        ...
        {/* The padlock, and the host beside it. */}
        ...
        {mode === "default" ? (
          <>
            ...
            {/* Shield, inside the left edge of the pill; reload, inside the right. */}
            <g className="mix-blend-luminosity">
              <path
      ...
      {/* The hairline between the toolbar and the page, which upstream draws as
          the 1px gap its screen mask leaves at y=52. */}
      <div className="border-t border-line">{screen}</div>
    </div>
  );
}
